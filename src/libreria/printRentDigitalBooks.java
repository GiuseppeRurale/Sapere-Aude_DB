...
public class printRentDigitalBooks extends JFrame {
	 ...
	 * Create the frame.
	 */
	public printRentDigitalBooks() {
		setType(Type.UTILITY);
		setTitle("OPERE DIGITALI");
		...
		JLabel indexLabel = new JLabel("Catalogo Opere Digitali:");
		...
		try {
			String b = "SELECT idOpera, opera.nome, mediaVoti, autore.nome, numeroPagine, linguaTesto, costoNoleggio FROM opera INNER JOIN operadigitale INNER JOIN autore ON idOpera = opera && autore=idAutore;";
			...
			String id, nome, voto, autore, pagine, lingua, prezzo;
			while(rso.next()) {
				id = rso.getString(1);
				nome = rso.getString(2);
				voto = rso.getString(3);
				autore = rso.getString(4);
				pagine = rso.getString(5);
				lingua = rso.getString(6);
				prezzo = rso.getString(7);
				String[] row = {id, nome, voto, autore, pagine, lingua, prezzo};
				model.addRow(row);
			}
			s.close();
