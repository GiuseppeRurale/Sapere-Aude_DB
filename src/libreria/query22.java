...
public class query22 extends JFrame {
	...
	public query22() {
		...
		setTitle("OPERE DIGITALI ACQUISTATE");
		...
		JLabel indexLabel = new JLabel("Le opere digitali che hai acquistato:");
		...
		try {
			String l = "SELECT opera.nome, mediaVoti, autore.nome, categoria, numeroPagine, linguaTesto FROM ordine INNER JOIN acquistopera INNER JOIN opera INNER JOIN autore INNER JOIN operadigitale ON idOrdine = ordine && acquistopera.opera = idOpera && autore = idAutore && operadigitale.opera = idOpera WHERE accountref = \""+Login.user+"\" && stato = 1 && tipoOpera = 0;";
			...
			String nome, voto, autore, categoria, pagine, lingua;
			while(rs.next()) {
				nome = rs.getString(1);
				voto = rs.getString(2);
				autore = rs.getString(3);
				categoria = rs.getString(4);
				pagine = rs.getString(5);
				lingua = rs.getString(6);
				String[] row = {nome, voto, autore, categoria, pagine, lingua};
				model.addRow(row);
			}
			s.close();
