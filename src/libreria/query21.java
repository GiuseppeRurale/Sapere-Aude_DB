...
public class query21 extends JFrame {
	...
	public query21() {
		...
		JLabel indexLabel = new JLabel("Le opere che hai noleggiato:");
		...
		try {
			String l = "SELECT opera.nome, mediaVoti, autore.nome, categoria, dataFineNoleggio FROM ordine INNER JOIN noleggiopera INNER JOIN opera INNER JOIN autore ON idOrdine = ordine && opera = idOpera && autore = idAutore WHERE accountref = \""+Login.user+"\" && stato = 1;";
			...
			String nome, voto, autore, categoria, end;
			while(rs.next()) {
				nome = rs.getString(1);
				voto = rs.getString(2);
				...
				String[] row = {nome, voto, autore, categoria, end};
				model.addRow(row);
			}
			s.close();
