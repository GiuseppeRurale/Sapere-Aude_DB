...
public class insertQuery13 extends JFrame {
	 ...
	 * Create the frame.
	 */
	public insertQuery13() {
		setTitle("NOLEGGIO AUDIOLIBRO");
		...
		JLabel operaLabel = new JLabel("Inserisci l'ID dell'audiolibro da noleggiare:");
		...
		catalogoButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				EventQueue.invokeLater(new Runnable() {
					public void run() {
						try {
							printRentAudioBooks frame = new printRentAudioBooks();
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					String query13 = "INSERT INTO noleggiopera(dataInizioNoleggio, dataFineNoleggio, tipoOpera, opera, ordine) VALUES(NOW(), DATE_ADD(dataInizioNoleggio, INTERVAL 90 DAY), 1, ?, ?);";
					PreparedStatement ps13 = Login.con.prepareStatement(query13);
					ps13.setString(1, operaField.getText());
					ps13.setString(2, ordineField.getText());
					String query132 = "UPDATE ordine SET importoTotale = importoTotale + (SELECT costoNoleggio FROM audiolibro WHERE opera = ?) WHERE idOrdine = ?;";
					PreparedStatement ps132 = Login.con.prepareStatement(query132);
					ps132.setString(1, operaField.getText());
					ps132.setString(2, ordineField.getText());
					int r = ps13.executeUpdate();
					int r2 = ps132.executeUpdate();
					if(r!=0 && r2!=0) {
