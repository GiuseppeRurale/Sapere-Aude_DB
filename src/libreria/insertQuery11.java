...
public class insertQuery11 extends JFrame {
	 ...
	 * Create the frame.
	 */
	public insertQuery11() {
		setTitle("NOLEGGIO OPERA DIGITALE");
		...
		JLabel ordineLabel = new JLabel("Inserisci l'ID dell'ordine in cui aggiungere il noleggio:");
		ordineLabel.setFont(new Font("Tahoma", Font.BOLD, 14));
		ordineLabel.setBounds(10, 52, 368, 27);
		contentPane.add(ordineLabel);
		
		JLabel operaLabel = new JLabel("Inserisci l'ID dell'opera digitale da noleggiare:");
		...
		JButton catalogoButton = new JButton("CATALOGO OPERE DIGITALI");
		...
		catalogoButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				EventQueue.invokeLater(new Runnable() {
					public void run() {
						try {
							printRentDigitalBooks frame = new printRentDigitalBooks();
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					String query11 = "INSERT INTO noleggiopera(dataInizioNoleggio, dataFineNoleggio, tipoOpera, opera, ordine) VALUES(NOW(), DATE_ADD(dataInizioNoleggio, INTERVAL 90 DAY), 0, ?, ?);";
					PreparedStatement ps11 = Login.con.prepareStatement(query11);
					ps11.setString(1, operaField.getText());
					ps11.setString(2, ordineField.getText());
					String query112 = "UPDATE ordine SET importoTotale = importoTotale + (SELECT costoNoleggio FROM operadigitale WHERE opera = ?) WHERE idOrdine = ?;";
					PreparedStatement ps112 = Login.con.prepareStatement(query112);
					ps112.setString(1, operaField.getText());
					ps112.setString(2, ordineField.getText());
					int r = ps11.executeUpdate();
					int r2 = ps112.executeUpdate();
					if(r!=0 && r2!=0) {
					JOptionPane.showMessageDialog(addButton, "NOLEGGIO AGGIUNTO ALL'ORDINE");
					ps11.close();
					setVisible(false);
					dispose();
					}
					else
						JOptionPane.showMessageDialog(addButton, "ERRORE, NOLEGGIO NON AGGIUNTO ALL'ORDINE");
